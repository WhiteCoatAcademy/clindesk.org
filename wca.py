...
if os.environ.get('SUPERVISOR_PROCESS_NAME', False) == 'wca-staging':
    app.config['STAGING'] = True
else:
    app.config['STATIC_ROOT'] = 'http://static.whitecoatacademy.org/s/'
...
if not app.debug:
    ...
    mail_handler = SMTPHandler('aspmx.l.google.com',
                               ...
                               ADMINS, 'WCA Prod Failure')
    mail_handler.setLevel(logging.ERROR)
    app.logger.addHandler(mail_handler)

# Create a static() handler and send content to static.whitecoatacademy.org
def static(path):
...
@app.route("/")
def welcome():
    return render_template('wca_index.html')
...
def github_pull_on_commit():
    if request.environ['HTTP_X_REAL_IP'] in ('207.97.227.253', '50.57.128.197', '108.171.174.178'):
        if app.config.get('STAGING', False):
            ...
            os.system('supervisorctl restart wca-staging')
            return "Pulling."
